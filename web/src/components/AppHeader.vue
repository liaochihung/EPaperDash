...
const emit = defineEmits(['update:selectedDisplay', 'update:selectedColorMode', 'connect', 'upload', 'open-settings']);
...
<template>
  <header class="h-14 bg-white border-b border-gray-200 flex items-center px-6 justify-between shadow-sm z-20 shrink-0">
    ...
    <div class="flex items-center space-x-3">
      ...
      <button @click="$emit('open-settings')" :disabled="!isConnected" :class="{'opacity-50 cursor-not-allowed': !isConnected}" class="px-4 py-1.5 bg-gray-100 text-gray-700 border border-gray-300 rounded-md text-sm hover:bg-gray-200 font-medium transition-colors shadow-sm flex items-center" aria-label="Device Settings">
